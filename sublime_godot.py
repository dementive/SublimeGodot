...
def to_markdown(text):
    ...
    if start_code_block != -1 and end_code_block != -1:
        code_block_content = text[start_code_block:end_code_block].strip()
...
class GodotClassDocumentation:
    ...
    def get_class_info(self):
        ...
        if desc is not None and desc.text:
            self.brief_description = desc.text.strip()

        desc = self.root.find('description')
        if desc is not None and desc.text:
            self.full_description = desc.text.strip()
    ...
    def get_constants_info(self):
        ...
        for constant in constants.findall('constant'):
            ...
            description = ""
            if constant.text:
                description = constant.text.strip()
